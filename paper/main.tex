...
transformer layers of three models — XLM-RoBERTa-base (13 layers),
XLM-RoBERTa-large (25 layers), and mBERT (13 layers) — using two independent
...
measurement. Both XLM-RoBERTa variants produce near-static representations for
all 30 professions tested (mean pairwise cosine similarity $>0.91$), yet exhibit
substantial male bias in mid-to-deep layers. XLM-RoBERTa-base peaks at
layer~9 ($+2.99$) in a single-phase accumulation pattern; XLM-RoBERTa-large
exhibits a two-phase double-peak structure (layer~13: $+2.99$, layer~23: $+3.46$),
with stronger embedding-level bias at layer~0 ($+1.29$ vs $+0.20$). Comparing
static embedding bias with contextual bias reveals two qualitatively distinct
mechanisms: \emph{embedding-level bias} and \emph{attention-level bias}.
ProjBias and CosDiff estimators agree near-perfectly across all layers in both
XLM-RoBERTa models (Spearman $\rho \geq 0.992$, $p < 10^{-26}$), and
batch-stability validation confirms that bias estimates are fully converged
and sampling-independent.
\end{abstract}
...
We evaluate three multilingual transformer models, all used strictly in inference
...
Table~\ref{tab:model_compare} summarises key metrics across the three models.
...
\toprule
\textbf{Model} & \textbf{Layers} & \textbf{Ti vocab} & \textbf{Peak layer} & \textbf{Peak proj} & \textbf{Min Spearman $\rho$} \\
\midrule
mBERT                & 13 & \textbf{[UNK]} & --- & $\approx -3.21^*$ & n/a \\
XLM-RoBERTa-base     & 13 & subword        & 9   & $+2.99$            & $0.992$ \\
XLM-RoBERTa-large    & 25 & subword        & 23  & $+3.46$            & $0.992$ \\
\bottomrule
\end{tabular}
\caption{Model comparison on Tigrigna gender bias pipeline. $^*$mBERT projects
all 30 professions identically to $\approx -3.21$ because every Tigrigna token
maps to \texttt{[UNK]}; the value reflects the single [UNK] embedding's position
relative to the anchor geometry and carries no profession-specific information.
Ti vocab = whether the tokeniser produces meaningful Tigrigna subwords.}
...
\begin{enumerate}
  ...
  \item \textbf{Perfect metric agreement simplifies reporting.} Spearman
    $\rho \geq 0.992$ between ProjBias and CosDiff at every layer, in both
    XLM-RoBERTa models, means both metrics are interchangeable for ranking
    professions by bias magnitude regardless of model scale.
\end{enumerate}
